...
// @Accept json
// @Produce json
// @Param request body dto.MidtransCallbackPayload true "Midtrans callback payload"
...
func (h *PaymentHandler) UpdatePaymentStatus(c *gin.Context) {
	...
	status := dto.PaymentStatus(statusStr)

	// Validate status
	validStatuses := []dto.PaymentStatus{
		dto.PaymentStatusPending,
		dto.PaymentStatusPaid,
		dto.PaymentStatusFailed,
		dto.PaymentStatusCancelled,
		dto.PaymentStatusRefunded,
	}
...
// @Produce json
// @Security BearerAuth
// @Param request body dto.MidtransPaymentRequest true "Midtrans payment data"
// @Success 200 {object} dto.APIResponse
// @Failure 400 {object} dto.ErrorResponse
// @Failure 401 {object} dto.ErrorResponse
// @Failure 500 {object} dto.ErrorResponse
...
func (h *PaymentHandler) ProcessMidtransPayment(c *gin.Context) {
	var req dto.MidtransPaymentRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, model.CreateErrorResponse(
			"validation_error",
			"Invalid request data",
...
func (h *PaymentHandler) ListPayments(c *gin.Context) {
	...
	if paymentMethodStr := c.Query("payment_method"); paymentMethodStr != "" {
		paymentMethod := dto.PaymentMethod(paymentMethodStr)
