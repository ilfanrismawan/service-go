...
func (m *Metrics) RecordPaymentProcessed(payment *paymentDTO.Payment) {
	...
	m.PaymentAmount.WithLabelValues(string(payment.PaymentMethod)).Observe(float64(payment.Amount))
}

// RecordNotificationSent records notification sending metrics
func (m *Metrics) RecordNotificationSent(notification *model.Notification, channel string) {
